    ...
    for path in sys.env.PATH\gmatch '[^:]+'
      ...
      if dir.exists
        for child in *dir.children
          table.insert commands, child.basename
...
class ExternalCommandConsole
  ...
  complete: (text) =>
    words = @_parse text
    if #words == 0 or (#words == 1 and words[1].word != 'cd')
      match_text = if words[1] then words[1].word else ''
      return name: 'command', completions: @available_commands, :match_text, columns: {{style: 'string'}}
